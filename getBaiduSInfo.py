...
if __name__ == "__main__":
    ...
    for row in results:
        ...
        for item in items:
            ...
            item = item.replace('</span>', '')
            issn = item
        #得出citationsperdocument
        citationsperdocument = float(yingxiangyinzi)/2
        #得出selfcites-you
        selfcites =  round((float(yingxiangyinzi)/10) * float(beiyinliang),2)
        #得出citesperdoc-you
        citesperdoc =  round(0.5+(float(yingxiangyinzi)/10),2)
        #得出externalcitesperdoc-you
        externalcitesperdoc =  round(float(citesperdoc) - 0.08,2)
        sql_update = "update list_of_journal set yingxiangyinzi = "+str(yingxiangyinzi)+",sousuozhishu = "+str(sousuozhishu)+",fawenliang = "+str(fawenliang)+",beiyinliang = "+str(beiyinliang)+",citationsperdocument = "+str(citationsperdocument)+",selfcites = "+str(selfcites)+",externalcitesperdoc = "+str(externalcitesperdoc)+",citesperdoc = "+str(citesperdoc)+",issn = \""+str(issn)+"\" where id = "+str(row[0])
        cursor.execute(sql_update)  # 像sql语句传递参数
        db.commit()
        print(yingxiangyinzi+"**"+sousuozhishu+"**"+fawenliang+"**"+beiyinliang+"**"+issn+"**"+str(citationsperdocument)+"**"+str(selfcites)+"**"+str(externalcitesperdoc)+"**"+str(citesperdoc))
